...
def save_task_schedules():
    ...
    for idx, item in enumerate(body):
        ...
        if not isinstance(item, dict):
            return jsonify({'success': False, 'error': f'El elemento en posición {pos} no es un objeto válido.'}), 400
        task_id = str(item.get('task_id') or '').strip()
        if not task_id:
            return jsonify({'success': False, 'error': f'El elemento en posición {pos} no tiene task_id.'}), 400
